...
class RP_WCDPD_Settings
{
                        ...
                        'product_pricing_settings' => array(
                            ...
                            'children' => array(
                                ...
                                'product_pricing_display_regular_price' => array(
                                    ...
                                    'type'      => 'checkbox',
                                    'default'   => '1',
                                    'hint'      => esc_html__('Controls whether regular price is displayed next to adjusted price on shop pages.', 'rp_wcdpd'),
                                ),
                                'product_pricing_allow_coupons' => array(
                                    'title'     => esc_html__('Allow coupons when prices are changed', 'rp_wcdpd'),
                                    'type'      => 'checkbox',
                                    'default'   => '1',
                                    'hint'      => esc_html__('Controls whether regular coupons are allowed if at least one product pricing rule is applicable to cart items.', 'rp_wcdpd'),
                                ),
                            ),
                        ),
                        'cart_discounts_settings' => array(
                            ...
                            'children' => array(
                                ...
                                'cart_discounts_apply_with_individual_use_coupons' => array(
                                    ...
                                    'type'      => 'checkbox',
                                    'default'   => '1',
                                    'hint'      => esc_html__('Controls whether cart discounts should be applied if individual use coupon is applied to cart.', 'rp_wcdpd'),
                                ),
                                'cart_discounts_allow_coupons' => array(
                                    'title'     => esc_html__('Allow coupons with cart discounts', 'rp_wcdpd'),
                                    'type'      => 'checkbox',
                                    'default'   => '1',
                                    'hint'      => esc_html__('Controls whether regular coupons are allowed if at least one cart discount is applicable to cart.', 'rp_wcdpd'),
                                ),
                            ),
                        ),
                    ...
                    switch($field['type']) {
                        ...
                        case 'number':
                            if (isset($input[$full_key]) && is_numeric($input[$full_key])) {
                                $output[$field_key] = (int) trim($input[$full_key]);
                            }
                            else {
                                $output[$field_key] = '';
                            }
                        ...
                        case 'decimal':
                            if (isset($input[$full_key]) && is_numeric($input[$full_key])) {
                                $output[$field_key] = (float) trim($input[$full_key]);
                            }
                            else {
                                $output[$field_key] = '';
                            }
    ...
    public function maybe_display_migration_notice()
    {
        ...
        // 2021-09-02 Security vulnerability notice
        if ($notice = get_option('rp_wcdpd_migration_notice_2021_09_security')) {

            // Print
            printf('<div class="update-nag rp_wcdpd_migration_notice"><h3>Warning!</h3>' . $notice . '<p><a href="%s">Find Out More</a>&nbsp;&nbsp;&nbsp;<a href="%s">Contact Support</a>&nbsp;&nbsp;&nbsp;<a href="%s">Hide this notice</a></p></div>', 'http://url.rightpress.net/wcdpd-security-announcement', 'http://url.rightpress.net/new-support-ticket', add_query_arg('rp_wcdpd_hide_migration_notice_2021_09_security', '1'));
    ...
    public function hide_migration_notice()
    {
        ...
        // 2021-09-02 Security vulnerability notice
        if (!empty($_REQUEST['rp_wcdpd_hide_migration_notice_2021_09_security'])) {
            delete_option('rp_wcdpd_migration_notice_2021_09_security');
            wp_redirect(remove_query_arg('rp_wcdpd_hide_migration_notice_2021_09_security'));
            exit;
        }
    }
    ...
    public function import()
    {
        ...
        // Verify nonce
        if (!isset($_REQUEST['_wpnonce']) || !wp_verify_nonce($_REQUEST['_wpnonce'], 'rp_wcdpd_settings_group_settings-options')) {
            exit;
        }
