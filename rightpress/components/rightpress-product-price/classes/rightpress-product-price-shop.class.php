...
final class RightPress_Product_Price_Shop
{
    ...
    public function maybe_change_price($price, $product)
    {
        ...
        if ($this->skip_cache($product)) {
            ...
            // Maybe skip temporary cache
            if ($this->skip_temporary_cache($product)) {
                return $this->calculate_price($price, $price_type, $product);
            }

            // Get price from temporary cache
            return $this->get_price_for_product_from_temporary_cache($price, $price_type, $product);
        }
    ...
    /**
     * Get price for product from temporary cache
     *
     * @access public
     * @param float $price
     * @param string $price_type
     ...
     * @return float
     */
    public function get_price_for_product_from_temporary_cache($price, $price_type, $product)
    {

        // Get temporary cache price hash
        $temporary_cache_price_hash = $this->get_price_hash($product, $price, $price_type) . '_' . RightPress_Help::get_hash(false, RightPress_Product_Price_Cart::get_environment_variables());

        // Price not in temporary cache
        if (!isset($this->temporary_cache[$temporary_cache_price_hash])) {

            // Calculate price
            $temporary_cached_price = $this->calculate_price($price, $price_type, $product);

            // Store price in temporary cache
            $this->temporary_cache[$temporary_cache_price_hash] = $temporary_cached_price;
        }

        // Return price from temporary cache
        return $this->temporary_cache[$temporary_cache_price_hash];
    }
    ...
    /**
     * Maybe skip temporary cache to calculate fresh prices for current function call
     *
     * If true, prices won't be read from temporary cache
     *
     * @access public
     * @param object $product
     * @return bool
     */
    public function skip_temporary_cache($product)
    {

        // Allow plugins and 3rd party developers to skip temporary caching
        return apply_filters('rightpress_product_price_shop_skip_temporary_cache', false, $product);
    }
